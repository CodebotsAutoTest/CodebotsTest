...
namespace APITests.EntityObjects.Models
{
	...
	{
		// 
		public String Assad { get; set; }

		/// <summary>
		/// Incoming one to many reference
		/// </summary>
		/// <see cref="Lm2348.Models.A"/>
		...
		public BEntity(ConfigureOptions option)
		{
			Configure(option);
			InitialiseAttributes();
			InitialiseReferences();
		...
		private static string GetInvalidAId(string validator)
		{
			switch (validator)
			{
				case "Required":
					return "";
				default:
					throw new Exception($"Cannot find validator {validator} for attribute bssssdasd");
			}
		}
		...
		public override void SetReferences (Dictionary<string, ICollection<Guid>> entityReferences)
		{
			foreach (var (key, guidCollection) in entityReferences)
			{
				...
				{
					case "AId":
						ReferenceIdDictionary.Add("AId", guidCollection.FirstOrDefault());
		...
		// TODO needs some warning if trying to get an invalid entity, and the entity
		// attributes don't actually have any validators to violate.
		public static BEntity GetEntity(bool isValid, string fixedValue = null)
		{
		...
		public static BEntity GetInvalidEntity()
		{
			var bEntity = new BEntity
			{
			};
			return bEntity;
		}
		...
		/// Gets an entity with attributes that conform to any attribute validators.
		/// </summary>
		public static BEntity GetValidEntity(string fixedStrValue = null)
		{
			var bEntity = new BEntity
			{

				Assad = (!string.IsNullOrWhiteSpace(fixedStrValue) && fixedStrValue.Length > 0 && fixedStrValue.Length <= 255) ? fixedStrValue : DataUtils.RandString(),
			};
		...
		{
			return SaveToDB<Lm2348.Models.BEntity>(BEntityDto.Convert(this));
		}
	}
}
